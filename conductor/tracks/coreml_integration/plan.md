...
- [x] Task: Write the Swift class implementation to load the `yolov8n` CoreML model using `VNCoreMLModel`.
- [x] Task: Create an exposed Tauri command in Swift (`run_inference`) that accepts base64 image data or raw byte arrays.
- [x] Task: Implement `VNCoreMLRequest` logic to process the image and extract bounding boxes.
